...
public class MainController implements Initializable {
    ...
    private void generatePdfOverdueReport() {
        ...
        try {
            ...
            addStatsRowVertical(statsTable, "Всего просроченных ТО:", String.valueOf(overdueList.size()),
                    new Font(baseFont, 10, Font.BOLD), new Font(baseFont, 12, Font.BOLD, new BaseColor(0, 102, 204)));

            addStatsRowVertical(statsTable, "Средний срок просрочки:", String.format("%.1f дней", avgOverdue),
                    new Font(baseFont, 10, Font.BOLD), new Font(baseFont, 12, Font.BOLD, new BaseColor(0, 102, 204)));
            ...
            for (MaintenanceSchedule ms : overdueList) {
                ...
                String nextDueStr = ms.getNextDue() != null ?
                        ms.getNextDue().format(DateTimeFormatter.ofPattern("dd.MM.yyyy")) : "Не указано";

                String lastDoneStr = ms.getLastDone() != null ?
                        ms.getLastDone().format(DateTimeFormatter.ofPattern("dd.MM.yyyy")) : "Не указано";

                long daysOverdue = ms.getNextDue() != null ?
                        ChronoUnit.DAYS.between(ms.getNextDue(), today) : 0;

                mainTable.addCell(createCell(machineModel, contentFont, Element.ALIGN_LEFT));
                mainTable.addCell(createCell(typeName, contentFont, Element.ALIGN_LEFT));
                mainTable.addCell(createCell(nextDueStr, contentFont, Element.ALIGN_CENTER));
                mainTable.addCell(createCell(lastDoneStr, contentFont, Element.ALIGN_CENTER));
    ...
    private BaseFont loadFont() throws Exception {
        try {
            ...
            return BaseFont.createFont(BaseFont.COURIER, BaseFont.CP1252, BaseFont.EMBEDDED);
        }
    }
